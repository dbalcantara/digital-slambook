...
class FriendUpdatePage extends StatefulWidget {
  final String friendId;

  const FriendUpdatePage({Key? key, required this.friendId}) : super(key: key);

  @override
  _FriendUpdatePageState createState() => _FriendUpdatePageState();
}

class _FriendUpdatePageState extends State<FriendUpdatePage> {
  ...
  void initState() {
    ...
    nicknameController = TextEditingController(text: friend['nickname']);
    ageController = TextEditingController(text: friend['age'].toString()); // Ensure age is treated as String
    single = friend['single'] ?? false; // Provide default value
    happinessLevel = (friend['happinessLevel'] ?? 5).toDouble(); // Provide default value
                  ...
                  ElevatedButton(
                    onPressed: () {
                      if (_formKey.currentState!.validate()) {
                        ...
                        context.read<FriendsProvider>().updateFriend(widget.friendId, friendData);
